...
class PrimerParcial2021 {
	...
	void queSePuedaCrearUnVideojuego() {
		...
		// Ejecución
		Videojuego juego = new Videojuego(CONSOLA_ESPERADA);
	...
	void queSePuedaCrearUnLibro() {
		...
		// Ejecución
		Libro libro = new Libro(AUTOR_ESPERADO, EDITORIAL_ESPERADA);
	...
	void queUnaPeliculaSeaVendible() {
		...
		// Ejecución
		Vendible pelicula = new Pelicula(GENERO_ESPERADO, ANO_DE_ESTRENO_ESPERADO, DIRECTOR_ESPERADO,PRECIO_VENTA);
	...
	void queSePuedaVenderUnLibro() {
		...
		Producto nuevoProducto = new Libro(AUTOR_ESPERADO, EDITORIAL_ESPERADA);
		Cliente nuevoCliente = new Cliente(CODIGO_ESPERADO, APELLIDO_ESPERADO, NOMBRE_ESPERADO, EDAD_ESPERADA);
